...
int main(int argc, char* argv[])
{
        ...
        if (vm.count("front") || vm.count("back")) {
                ...
                isolator->extra_passes =
                                vm.count("extra-passes") ?
                                                vm["extra-passes"].as<int>() :
                                                0;
        }
        ...
        if (vm.count("outline")
                        || (vm.count("drill") && vm.count("milldrill"))) {
                cutter = shared_ptr<Cutter>(new Cutter());
                cutter->tool_diameter = vm["cutter-diameter"].as<double>()
                                * unit;
        ...
        if (vm.count("preamble")) {
                ...
                if (!in.good()) {
                        cerr << "Cannot read preamble file \"" << name << "\""
                                        << endl;
                        exit(1);
                }
        ...
        shared_ptr<Board> board(
                        new Board(vm["dpi"].as<int>(), vm.count("fill-outline"),
        ...
        try {
                ...
                try {
                        ...
                        board->prepareLayer("outline", importer, cutter,
                                        !vm.count("front"),
                                        vm.count("mirror-absolute"));
                        cout << "done\n";
                } catch (import_exception& i) {
        ...
        try {
                ...
                cout << "Calculated board dimensions: " << board->get_width()
                                << "in x " << board->get_height() << "in"
                                << endl;

                //SVG EXPORTER
                if (vm.count("svg")) {
                        cout << "Create SVG File ... " << vm["svg"].as<string>()
                                        << endl;
        ...
        if (vm.count("drill")) {
                ...
                try {
                        ExcellonProcessor ep(vm["drill"].as<string>(),
                                        board->get_min_x() + board->get_max_x(),
                        ...
                        if (vm.count("milldrill"))
                                ep.export_ngc(vm["drill-output"].as<string>(),
                                                cutter,
                                                !vm.count("drill-front"),
                                                vm.count("mirror-absolute"),
                                                vm["onedrill"].as<bool>());
                        else
                                ep.export_ngc(vm["drill-output"].as<string>(),
