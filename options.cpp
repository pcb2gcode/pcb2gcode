...
/******************************************************************************/
options::options() :
		cli_options("command line only options"), cfg_options(
	...
	cfg_options.add_options()("front", po::value<string>(),
			"front side RS274-X .gbr")("back", po::value<string>(),
			"back side RS274-X .gbr")("outline", po::value<string>(),
			"pcb outline polygon RS274-X .gbr")("drill", po::value<string>(),
			"Excellon drill file\n")

	("svg", po::value<string>(), "SVG output file. EXPERIMENTAL\n")

	("zwork", po::value<double>(),
			"milling depth in inches (Z-coordinate while engraving)")("zsafe",
			po::value<double>(),
			"safety height (Z-coordinate during rapid moves)")("offset",
			po::value<double>(),
			...
			"milldrill", "drill using the mill head")("extra-passes",
			po::value<int>(),
	...
	("fill-outline", po::value<bool>()->zero_tokens(),
			"accept a contour instead of a polygon as outline")("outline-width",
			po::value<double>(), "width of the outline")("cutter-diameter",
			po::value<double>(),
			"diameter of the end mill used for cutting out the PCB")("zcut",
			po::value<double>(), "PCB cutting depth in inches.")("cut-feed",
			po::value<double>(), "PCB cutting feed")("cut-speed",
			po::value<int>(), "PCB cutting spindle speed")("cut-infeed",
			po::value<double>(),
			"Maximum cutting depth; PCB may be cut in multiple passes\n")

	("zdrill", po::value<double>(), "drill depth")("zchange",
			po::value<double>(), "tool changing height")("drill-feed",
			po::value<double>(), "drill feed; ipm")("drill-speed",
			po::value<int>(), "spindle rpm when drilling")("drill-front",
			po::value<bool>()->zero_tokens(),
			"drill through the front side of board")("onedrill",
			po::value<bool>()->default_value(false)->zero_tokens()->implicit_value(
					true), "use only one drill bit size\n")
	...
	("metric",
			...
			"metricoutput",
			po::value<bool>()->default_value(false)->zero_tokens()->implicit_value(
					true), "use metric units for output")("dpi",
			po::value<int>()->default_value(1000),
			"virtual photoplot resolution")("g64", po::value<double>(),
			"maximum deviation from toolpath")("mirror-absolute",
			po::value<bool>()->zero_tokens(),
	...
	("basename", po::value<string>(), "prefix for default output file names")(
			...
			"output file for front layer")("back-output",
			po::value<string>()->default_value("back.ngc"),
			"output file for back layer")("outline-output",
			po::value<string>()->default_value("outline.ngc"),
			"output file for outline")("drill-output",
			po::value<string>()->default_value("drill.ngc"),
			"output file for drilling\n")

	("preamble", po::value<string>(), "gcode preamble file")("postamble",
			po::value<string>(), "gcode postamble file");
}
