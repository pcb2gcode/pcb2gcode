...
NGC_Exporter::export_all( boost::program_options::variables_map& options )
{
  ...
  if( metricinput )  g64/=25.4;                                       //convert to inches if needed 
...
NGC_Exporter::export_layer( shared_ptr<Layer> layer, string of_name )
{
  ...
  of << "G64 P" << get_tolerance() * factor << " ( set maximum deviation from commanded toolpath )\n"
     << endl;
  ...
  BOOST_FOREACH( shared_ptr<icoords> path, layer->get_toolpaths() ) {
    ...
    if( cutter && cutter->do_steps ) {
      ...
      while( z >= mill->zwork ) {
        of << "G01 Z" << z * factor << " F" << mill->feed * factor << " ( plunge. )\n";
        of << "G04 P0 ( dwell for no time -- G64 should not smooth over this point )\n";
      ...
      // isolating
      of << "G01 Z" << mill->zwork * factor << " F" << mill->feed * factor << " ( plunge. )\n";
      of << "G04 P0 ( dwell for no time -- G64 should not smooth over this point )\n";
