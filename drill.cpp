...
\version
        1.1.x - 2013 - Erik Schuster - erik@muenchen-ist-toll.de\n
...
ExcellonProcessor::ExcellonProcessor (string drillfile,
                                      ...
                                      bool _metricoutput) : board_width (board_width)
{
        bDoSVG = false;
        project = gerbv_create_project();
        metricoutput = _metricoutput;         //set flag for metric output
                                    ...
                                    bool mirrored, bool mirror_absolute)
{
        ...
        for (map<int, drillbit>::const_iterator it = bits->begin(); it != bits->end(); it++) {
                of << "G00 Z" << driller->zchange * factor << " ( Retract )\n"
                   << "T" << it->first << endl
                   << "M5      ( Spindle stop.                )\n"
                   << "M6      ( Tool change.                 )\n"
                ...
                of << "G81 R" << driller->zsafe * factor << " Z" << driller->zwork * factor << " F" << driller->feed * factor
                   << " X" << (mirrored ? double_mirror_axis - coord_iter->first : coord_iter->first) * factor << " Y" << coord_iter->second * factor << endl;
                ++coord_iter;

                while (coord_iter != drill_coords.end()) {
                        of << "X" << (mirrored ? double_mirror_axis - coord_iter->first : coord_iter->first) * factor << " Y" << coord_iter->second * factor << endl;
        ...
        // retract, end
        of << "G00 Z" << driller->zchange * factor << " ( All done -- retract )\n" << endl;
                                  ...
                                  shared_ptr<Cutter> cutter, float holediameter)
{
        ...
        if (cutdiameter * 1.001 >= holediameter) {
                of << "G0 X" << x * factor << " Y" << y * factor << endl;
                ...
                of << "G0 X" << (x + millr) * factor << " Y" << y << endl;
                ...
                while (z >= cutter->zwork) {
                        ...
                        of << "G2 I" << -millr * factor << " J0" << endl;
                        z -= z_step;
                        stepcount--;
                }

                of << "G0 Z" << cutter->zsafe * factor << endl << endl;
        }
}
                                    ...
                                    bool mirrored, bool mirror_absolute)
{
        ...
        of << "#50=" << target->zwork * factor << " ; zwork" << endl;
        of << "#51=" << target->zsafe * factor << " ; zsafe" << endl;
        of << "#52=" << target->stepsize * factor << " ; stepsize" << endl;
        ...
        // retract, end
        of << "G00 Z" << target->zchange * factor << " ( All done -- retract )\n" << endl;
