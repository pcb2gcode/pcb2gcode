...
/*!
 ...
 MERCHANTABILITY or FITNESSgit push origin master FOR A PARTICULAR PURPOSE.  See the
...
void ExcellonProcessor::export_ngc(const string of_name,
                shared_ptr<Driller> driller, bool mirrored,
                bool mirror_absolute, bool onedrill)
{
        ...
        for (map<int, drillbit>::const_iterator it = bits->begin();
                ...
                if ((onedrill == true) && (it != bits->begin())) {
                        of
                                        << "(Drill change skipped due to 'onedrill' parameter.)\n"
                                        << endl;
                } else {
                        of << "G00 Z" << driller->zchange * cfactor
                                        << " ( Retract )\n" << "T" << it->first
                                        << endl
                                        << "M5      ( Spindle stop.                )\n"
                                        << "M6      ( Tool change.                 )\n"
                ...
                of << "G81 R" << driller->zsafe * cfactor << " Z"
                                << driller->zwork * cfactor << " F"
                                << driller->feed * cfactor << " X"
                                << (mirrored ? double_mirror_axis
                                                               - coord_iter->first :
                                               coord_iter->first) * cfactor
                ...
                shared_ptr<Cutter> cutter, float holediameter)
{
        ...
        if (cutdiameter * 1.001 >= holediameter) {
                ...
                //z_step=0.01;
                double z = cutter->zwork
                                + z_step * abs(int(cutter->zwork / z_step));
                ...
                bool onedrill)
{
        ...
        for (map<int, drillbit>::const_iterator it = bits->begin();
                ...
                millhole(of, board_width - coord_iter->first,
        ...
        // retract, end
        of << "G00 Z" << target->zchange * cfactor
                        << " ( All done -- retract )\n" << endl;
...
void ExcellonProcessor::parse_bits()
{
        ...
        for (gerbv_drill_list_t* currentDrill =
                        ...
                        currentDrill; currentDrill = currentDrill->next) {
                drillbit curBit;
                ...
                bits->insert(
                                pair<int, drillbit>(currentDrill->drill_num,
                                                curBit));
        }
}
...
void ExcellonProcessor::parse_holes()
{
        ...
        for (gerbv_net_t* currentNet = project->file[0]->image->netlist;
                        currentNet; currentNet = currentNet->next) {
                if (currentNet->aperture != 0)
                        (*holes)[currentNet->aperture].push_back(
                                        icoordpair(currentNet->start_x,
                                                        currentNet->start_y));
        }
}
